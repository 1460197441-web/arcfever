...
const certificationProfile = {
  ...
  school: '中国海洋大学',
  college: '海洋地球科学学院',
  pendingMessage: '当前原型仅要求姓名与电话，学校和学院通过平台维护的认证列表选择。'
};
...
const chats = [
  {
    id: 'chat-001',
    sellerName: '王海宁',
    instrumentName: '岩芯薄片扫描仪',
    lastMessage: '明天上午可以现场看设备。',
    updatedAt: '10:26'
  },
  {
    id: 'chat-002',
...
const ratings = [
  {
    id: 'rate-001',
    orderId: 'ord-20260318-004',
    target: '便携式重金属检测仪',
    score: 5,
    content: '沟通顺畅，设备状态和描述一致。'
  }
];
...
function getCartDetail() {
  const list = cartItems.map((item) => {
    ...
    return {
      ...item,
      instrument,
      subtotal: instrument.price * item.quantity
    };
  });
...
function buildHomeData() {
  return {
    ...
    roleCards: [
      {
        title: '已认证个人用户',
        desc: '认证后兼具买家和卖家双重身份，可发布个人仪器，也能浏览并购买他人发布的仪器。'
      },
      {
        title: '管理者 / 平台方',
        desc: '负责学校学院维护、用户认证审核、违规信息处理和交易纠纷协调。'
      }
    ],
    ...
    stats: [
      ...
      { label: '待审核认证', value: `${adminStats.pendingCertification} 条` }
    ]
  };
}
...
function invokeMockService(action, payload = {}) {
  switch (action) {
    ...
    case 'getOrderPreview': {
      ...
      return {
        ...cart,
        contactName: certificationProfile.name,
        contactPhone: certificationProfile.phone
      };
    }
    case 'createOrder':
      return {
        success: true,
        orderId: 'ord-20260319-099',
        amount: getCartDetail().total
      };
    case 'getPaymentInfo':
      return {
        order: getOrderById(payload.orderId || orders[0].id)
      };
    case 'confirmPayment':
      return {
        success: true,
        orderId: payload.orderId || orders[0].id
      };
    case 'getChatList':
      return { list: chats };
    case 'getChatDetail':
    ...
    case 'submitCertification':
      return {
        success: true,
        status: '待审核',
        message: '认证信息已提交，管理员审核通过后即可发布仪器。'
      };
    case 'getMyInstruments':
      return {
        list: instruments.filter((item) => item.sellerId === 'u-001')
      };
    case 'publishInstrument':
      return {
        success: true,
        instrumentId: `ins-${Date.now()}`,
        message: '仪器发布成功，当前为演示环境，默认直接上架。'
      };
    case 'getInstrumentEditDetail':
      return {
        instrument: getInstrumentById(payload.id)
      };
    ...
    case 'getOrderDetail': {
      ...
      return {
        order,
        instrument: getInstrumentById(order.instrumentId)
      };
    }
    case 'getFavoriteList':
      return {
        list: instruments.filter((item) => favorites.includes(item.id))
      };
    case 'getRatingList':
      return {
        list: ratings
