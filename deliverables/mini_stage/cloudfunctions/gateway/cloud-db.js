...
async function getInstrumentsRaw() {
  ...
  return list.length ? list : invokeMockService('getInstruments', { category: '全部', keyword: '' }).list;
}

async function getInstrumentById(id) {
  const fromDb = await findOneById(COLLECTIONS.instruments, id);
  if (fromDb) return fromDb;
  const list = await getInstrumentsRaw();
...
async function findChatForOrder(order) {
  if (!order || !order.instrumentId || !order.buyerUserId) return null;
  return findChatByBuyerAndInstrument(order.buyerUserId, order.instrumentId);
}
...
async function ensureChatForOrder(order, instrument, user) {
  ...
  await db.collection(COLLECTIONS.chatMessages).add({
    data: {
      ...
      chatId: chat.id,
      from: 'system',
      text: `订单 ${order.id} 已创建，可在此同步排期、交付与验收信息。`,
...
async function createOrder(payload, wxContext) {
  ...
  await ensureChatForOrder(order, instrument, user);
...
async function ensureOrderAccess(order, user, actionName) {
  ...
  if (order.buyerUserId !== user._id && order.sellerId !== user._id && !isAdminUser(user)) {
    throw new Error(`${actionName || '当前操作'}无权访问该订单`);
  }
}
...
async function getOrderDetail(payload, wxContext) {
  ...
  await ensureOrderAccess(order, user, '查看订单');
  const instrument = order.instrumentId ? await getInstrumentById(order.instrumentId) : null;
  const chat =
    instrument && order.buyerUserId && order.sellerId
      ? await findChatByParticipants(order.buyerUserId, order.sellerId, instrument.id)
      : null;
  return {
    order: clone(order),
...
async function getPaymentInfo(payload, wxContext) {
  ...
  await ensureOrderAccess(order, user, '查看支付信息');
...
async function confirmPayment(payload, wxContext) {
  const user = await getCurrentUser(wxContext);
  const order = await getOrderById(payload.orderId);
  await ensureOrderAccess(order, user, '支付订单');
  if (order.status !== '待付款') throw new Error('订单当前状态不可支付');
  const status =
    order.tradeMode === '出售'
      ? '已付款待发货'
      : order.tradeMode === '远程实验咨询'
        ? '待平台联系'
        : '已付款待排期';
...
async function getChatList(wxContext) {
  ...
  for (const chat of chatMap.values()) {
    ...
    list.push({
      ...
      lastMessage: rows.length ? rows[rows.length - 1].text : '暂无消息',
